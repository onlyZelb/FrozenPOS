...
    wholeSale: 0,
    stockQuantity: 0,
    imagePath: "", // Retaining the path for display/reference
  });
  const [selectedImage, setSelectedImage] = useState(null); // State to hold the file object
...
  };
  ...
  const handleImageChange = (e) => {
      // Set the file object when a file is selected
...
    e.preventDefault();
    ...
    // 2. Conditionally append the image file
    if (selectedImage) {
        formData.append("file", selectedImage); // "file" must match your Spring Boot controller's @RequestParam name
    } else if (form.imagePath) {
        // If editing and no new file selected, send the existing path (optional, depending on backend logic)
...
      stockQuantity: product.stockQuantity || 0,
      imagePath: product.imagePath || "", // Load existing image path
    });
    // Clear any previously selected file when starting an edit
    setSelectedImage(null); 
...
        />
        ...
        <div className="flex-1 min-w-[200px]">
          <input
            type="file"
            name="image"
            onChange={handleImageChange} // Use separate handler for file
